...
- [ y ] 输入代码并滚动，高亮层与输入层同步滚动，不错位
- [ y ] 调整窗口大小，编辑器自适应，滚动同步依然正常
- [ y ] 切换深色/浅色主题，高亮颜色即时切换，无残留

## 2. 调试器 & 执行 (Debugger & Execution)

- [ y ] **Run (F5)**：全速运行，Tape 实时更新，IP 推进，状态显示 `Running`
- [ y ] **Step Over (F10)**：单步执行，Tape 更新，IP 推进，状态显示 `Paused`
- [ y ] **Step Back (F9)**：回退一步，Tape、IP、Output 恢复上一步状态，状态显示 `Paused`
- [ y ] **Reset**：重置所有状态（Tape = [0], IP = 0, Output = ''），会话保持，状态显示 `Ready`
- [ y ] **Done**：结束会话，代码区恢复可编辑，状态显示 `Idle`，断点清空
- [ y ] **Run + Pause**：Run 运行中点击 Run，暂停运行，状态显示 `Paused`；再次点击 Run，继续运行
- [ y ] **程序结束**：执行到代码末尾，状态显示 `Finished`
- [ y ] **Tape 限制**：程序 `>` 超过 10,000 个 cell，自动停止，状态栏显示错误信息
- [ y ] **Output 限制**：程序 `.` 超过 10,000 字符，自动停止，状态栏显示错误信息
